resource "kubectl_manifest" "alb-controller-crd" {
...
resource "kubectl_manifest" "alb-controller-crd-1" {
  depends_on = [kubectl_manifest.alb-controller-crd]
  yaml_body  = <<YAML
apiVersion: apiextensions.k8s.io/v1
kind: CustomResourceDefinition
metadata:
...
# Considering the load balancers rules quota it's better to use Ingress Groups
resource "helm_release" "aws-load-balancer-controller" {
  depends_on = [kubectl_manifest.alb-controller-crd-1]
