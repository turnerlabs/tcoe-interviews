...
describe("Testing the Dynamic table in the page", () => {
  ...
  it("Validate CPU data", async () => {
    await homePage.goToDynamicTable();
    let rowsWithData = await dynamicTable.getRowsLength();
    expect((await dynamicTable.getCpuData()).length).to.equal(rowsWithData);
  });  
  it("Validate the Memory data", async () => {
    await homePage.goToDynamicTable();
    let rowsWithData = await dynamicTable.getRowsLength();
    expect((await dynamicTable.getMemoryData()).length).to.equal(rowsWithData);
  });

  it("Validate the Network data", async () => {
    await homePage.goToDynamicTable();
    let rowsWithData = await dynamicTable.getRowsLength();
    expect((await dynamicTable.getNetworkData()).length).to.equal(rowsWithData);
  });
  it("Validate the Disk data", async () => {
    await homePage.goToDynamicTable();
    let rowsWithData = await dynamicTable.getRowsLength();
    expect((await dynamicTable.getDiskData()).length).to.equal(rowsWithData);
  });
});
